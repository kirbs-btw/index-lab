...
/// Configuration for the APEX index
...
impl ApexConfig {
    ...
    pub fn validate(&self) -> crate::error::Result<()> {
        if self.router_hidden_dim == 0 {
            return Err(crate::error::ApexError::InvalidConfig(
...
mod tests {
    ...
    fn test_high_recall_config_is_valid() {
        let config = ApexConfig::high_recall();
        assert!(config.validate().is_ok());
    }

    #[test]
    fn test_high_speed_config_is_valid() {
        let config = ApexConfig::high_speed();
        assert!(config.validate().is_ok());
    }

    #[test]
    fn test_energy_efficient_config_is_valid() {
        let config = ApexConfig::energy_efficient();
        assert!(config.validate().is_ok());
    }
}
