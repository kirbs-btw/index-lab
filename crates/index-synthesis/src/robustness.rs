...
//! 
//! ACTUALLY TRIGGERS router retraining when shift is detected
...
/// 
/// ACTUALLY TRIGGERS router retraining when shift detected
#[derive(Debug, Clone)]
pub struct ShiftDetector {
    tracker: DistributionTracker,
    threshold: f32,
    /// Last shift detection result (cached)
    last_shift_detected: bool,
}
...
impl ShiftDetector {
    pub fn new(window_size: usize, threshold: f32) -> Self {
        Self {
            ...
            threshold,
            last_shift_detected: false,
        }
    }
    
    pub fn update(&mut self, vector: &MultiModalVector) {
        self.tracker.update(vector);
        // Reset shift flag when new data arrives
        self.last_shift_detected = false;
    }
    
    /// Detect if distribution shift has occurred
    /// 
    /// ACTUALLY CALLED during insert/search operations
    /// Returns true if shift detected (should trigger router retraining)
    pub fn detect_shift(&mut self) -> anyhow::Result<bool> {
    ...
    /// Check if shift was detected in last check
    ...
    /// Reset the detector (after adaptation)
    pub fn reset(&mut self) {
        self.tracker = DistributionTracker::new(self.tracker.window_size);
        self.last_shift_detected = false;
    }
}
