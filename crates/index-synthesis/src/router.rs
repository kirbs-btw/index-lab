...
//! 
//! Uses RefCell for interior mutability so router can be updated
//! during standard search operations (fixes APEX issue)
...
pub struct MultiModalRouter {
    // Inner router wrapped in RefCell
...
impl ClusterRouter {
    ...
    fn predict(&self, query: &[f32]) -> Result<Vec<f32>> {
        if query.len() != self.input_dim {
            return Err(crate::error::SynthesisError::RouterTrainingError(
...
impl MultiModalRouter {
    ...
    /// 
    /// Can be called from immutable context
    ...
    /// 
    /// Can be called from immutable context (uses RefCell)
    pub fn update(&self, query: &MultiModalQuery, true_clusters: &[usize]) -> Result<()> {
...
mod tests {
    ...
    fn test_multimodal_router_dense_only() {
        let router = MultiModalRouter::new(
            Some(64),
            None,
            128,
            10,
